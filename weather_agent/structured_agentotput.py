...
class MyOutputformate(BaseModel):
    STEP: str = Field(..., description="The step of the response: START, PLAN, TOOL, OBSERVE, OUTPUT")
    ...
    output: (Optional[str]) = Field(None, description="The output from the tool if STEP is OBSERVE")
...
while True:
    ...
    final_results=response.choices[0].message.parsed
    
    # If response is a list, take first item
    if isinstance(final_results, list):
        final_results = final_results[0]

    if final_results.STEP =="START":
       print("Started: ",final_results.CONTENT)
       continue
    if final_results.STEP=="TOOL":
       tool_call=final_results.tool
       tool_input = final_results.input
       ...
       continue
    if final_results.STEP=="PLAN":
       print("PLAN: ",final_results.CONTENT)
       continue
    if final_results.STEP=="OUTPUT":
       print("OUTPUT: ",final_results.CONTENT)
       break
